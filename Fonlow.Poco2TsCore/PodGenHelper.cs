...
namespace Fonlow.Poco2Client
{
	...
	public static class PodGenHelper
	{
		...
		/// <summary>
		/// Adds JSON-serializable class registrations to a serializer context class within the specified namespace, creating
		/// the context class if it does not already exist.
		/// </summary>
		/// <remarks>The generated or updated class is named "AppJsonSerializerContext", is public and partial, and
		/// derives from System.Text.Json.Serialization.JsonSerializerContext. Each class name provided is registered using a
		/// JsonSerializable attribute. Repeated calls will add additional registrations to the same context class within the
		/// namespace.</remarks>
		/// <param name="ns">The CodeNamespace to which the serializer context class will be added or updated.</param>
		/// <param name="classNames">An array of class names representing types to register with the serializer context. Each name should correspond to
		/// a type to be included for JSON serialization.</param>
		/// <returns>A CodeTypeDeclaration representing the serializer context class with the specified class registrations. If the
		/// context class did not exist, it is created and added to the namespace.</returns>
