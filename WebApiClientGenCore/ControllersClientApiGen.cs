    ...
    public class ControllersClientApiGen : IDisposable
    {
        ...
        public CodeNamespaceEx[] CreateCodeDom(WebApiDescription[] descriptions)
        {
            ...
            foreach (var grouppedControllerDescriptions in controllersGroupByNamespace)
            {
                ...
                if (codeGenParameters.ClientApiOutputs.UseRefit)
                {
                    clientNamespace.Imports.Add(new CodeNamespaceImport("Refit"));
                }
        ...
        CodeTypeDeclaration CreateControllerClientInterface(CodeNamespace ns, string interfaceName)
        {
            var targetInterface = new CodeTypeDeclaration(interfaceName)
            {
                IsInterface = true,
            };

            ns.Types.Add(targetInterface);
