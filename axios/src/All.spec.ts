...
describe('SuperDemo API', () => {
	...
	it('getNullableDecimalNull', async () => {
		try {
			...
			expect(data).toEqual(''); // AXIOS return empty string rather than null. known behavior/quirk of Axios — debated in the community but not officially classified as a bug by the maintainers.
		} catch (error) {
			throw new Error(await errorResponseToString(error));
		}
	});
	...
	it('getNullPerson', async () => {
		try {
			...
			expect(data).toEqual(''); // empty string rather than null, a known feature of AXIOS.
		} catch (error) {
			throw new Error(await errorResponseToString(error));
		}
	});
