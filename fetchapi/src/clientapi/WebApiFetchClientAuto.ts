...
export namespace DemoWebApi_Controllers_Client {
	...
	export class Heroes {
		...
		post(name: string): Promise<DemoWebApi_Controllers_Client.Hero> {
			return fetch(this.baseUri + 'api/Heroes?name=' + (name == null ? '' : encodeURIComponent(name)), { method: 'post' }).then(d => d.json());
		}
		...
		postWithQuery(name: string): Promise<DemoWebApi_Controllers_Client.Hero> {
			return fetch(this.baseUri + 'api/Heroes/q?name=' + (name == null ? '' : encodeURIComponent(name)), { method: 'post' }).then(d => d.json());
		}
	...
	export class SuperDemo {
		...
		postActionResult(): Promise<string> {
			return fetch(this.baseUri + 'api/SuperDemo/ActionResult', { method: 'post' }).then(d => d.json());
		}
		...
		postWithQueryButEmptyBody(s: string, i: number): Promise<{item1: string, item2: number}> {
			return fetch(this.baseUri + 'api/SuperDemo/PostEmpty?s=' + (s == null ? '' : encodeURIComponent(s)) + '&i=' + i, { method: 'post' }).then(d => d.json());
		}
